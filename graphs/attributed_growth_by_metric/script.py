...
DEFAULT_OUTPUT_PATH = (
    REPO_ROOT
    / "outputs"
    / "attributed-growth-by-metric"
    / "attributed_growth_by_metric.xlsx"
)
...
DEFAULT_CONFIG = {
    "sectionNumber": "1",
    "sectionTitle": "Attributed Growth by Metric",
    "sheetName": "Attributed Metric Growth",
    ...
    "nativeMetricLabels": True,
    "useOverlayMetricLabels": False,
    ...
    "segments": [
        {"key": "NII", "label": "Net Interest Income", "color": "#001A52", "textColor": "#FFFFFF"},
        {
    ...
    "banks": [
        {
            ...
            "totalGrowthPct": 12,
            "brandColor": "#0B3A78",
            "metrics": {"NII": 5, "NON_INTEREST_INCOME": 7},
        },
        {
            "bank": "BMO",
            ...
            "totalGrowthPct": 9,
            "brandColor": "#0072A8",
            "metrics": {"NII": 3, "NON_INTEREST_INCOME": 6},
        },
        {
            "bank": "Scotia",
            ...
            "totalGrowthPct": 11,
            "brandColor": "#D71920",
            "metrics": {"NII": 4, "NON_INTEREST_INCOME": 7},
        },
        {
            "bank": "CIBC",
            ...
            "totalGrowthPct": 13,
            "brandColor": "#8B1020",
            "metrics": {"NII": 6, "NON_INTEREST_INCOME": 8},
        },
        {
            "bank": "National Bank",
            "shortName": "National",
            ...
            "attributed": True,
            "brandColor": "#C6002B",
            "metrics": {"NII": 9, "NON_INTEREST_INCOME": 6},
        },
        {
            "bank": "TD",
            ...
            "attributed": True,
            "brandColor": "#2E7D32",
            "metrics": {"NII": 6, "NON_INTEREST_INCOME": 6},
        },
    ],
}
...
def normalize_bank_record(bank: dict[str, Any]) -> dict[str, Any]:
    ...
    if "highlighted" in normalized:
        normalized["highlighted"] = to_bool(normalized["highlighted"])
    metric_input = bank.get("metrics") or bank.get("segments") or {}
    normalized["segments"] = {
        **deepcopy(defaults.get("metrics") or defaults.get("segments", {})),
        **{
            key: to_number(value)
            for key, value in deepcopy(metric_input).items()
        },
    }
    normalized["metrics"] = normalized["segments"]
    return normalized
    ...
    for row in rows:
        ...
        for segment in segments:
            segment_values[segment["key"]] = to_number(
                first_present(
                    (row.get("metrics") or {}).get(segment["key"]),
                    (row.get("metrics") or {}).get(segment["label"]),
        ...
        for target, keys in {
            ...
            "highlighted": ["highlighted", "Highlighted", "Highlight"],
        }.items():
            value = row_value(row, keys)
            if value not in (None, ""):
                normalized[target] = to_bool(value) if target == "highlighted" else value
...
def prepare_config(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    segments = []
    raw_segments = parsed.get("metrics") or parsed.get("segments") or default_segments
...
def get_format_spec() -> str:
    ...
    return """
Format the calculation results into one JSON object for an attributed growth by metric chart.

Return only JSON. Use this structure:
{
  "section_number": "1",
  "section_title": "Attributed Growth by Metric",
  "sheet_name": "Attributed Metric Growth",
...
- Metric values are contribution percentages, not dollar values.
...
- If the calculation payload stores metric values as objects, use the contribution or
  attribution percentage field for the metric value.
...
def normalize_formatted_segments(formatted: dict[str, Any]) -> list[dict[str, Any]]:
    raw_segments = (
        formatted.get("metrics")
        or formatted.get("metric_names")
        or formatted.get("segments")
        or formatted.get("segment_names")
    )
    if not raw_segments:
    ...
    for raw_segment in raw_segments:
        if isinstance(raw_segment, dict):
            label = first_present(
                ...
                raw_segment.get("metric"),
                raw_segment.get("metric_name"),
...
def from_formatted(formatted: dict[str, Any]) -> dict[str, Any]:
    ...
    for record in iter_formatted_bank_records(formatted):
        ...
        raw_segments = (
            record.get("metrics")
            or record.get("metric_contributions")
            or record.get("metricContributions")
            or record.get("segments")
        ...
        highlighted = first_present(record.get("highlighted"), record.get("highlight"))
        if highlighted not in (None, ""):
            bank["highlighted"] = to_bool(highlighted)
        if attributed_banks:
    ...
    config = {
        ...
        "segments": segments,
        "metrics": segments,
        "legendOrder": first_present(
            formatted.get("legendOrder"),
            formatted.get("legend_order"),
            DEFAULT_CONFIG["legendOrder"],
        ),
        "highlightBanks": first_present(
            formatted.get("highlightBanks"),
            formatted.get("highlight_banks"),
            DEFAULT_CONFIG["highlightBanks"],
        ),
        "highlightBox": first_present(
            formatted.get("highlightBox"),
            formatted.get("highlight_box"),
            DEFAULT_CONFIG["highlightBox"],
        ),
        "nativeMetricLabels": to_bool(
            first_present(
                formatted.get("nativeMetricLabels"),
                formatted.get("native_metric_labels"),
                formatted.get("nativeSegmentLabels"),
                formatted.get("native_segment_labels"),
                DEFAULT_CONFIG["nativeMetricLabels"],
            )
        ),
        "useOverlayMetricLabels": to_bool(
            first_present(
                formatted.get("useOverlayMetricLabels"),
                formatted.get("use_overlay_metric_labels"),
                formatted.get("useOverlaySegmentLabels"),
                formatted.get("use_overlay_segment_labels"),
                DEFAULT_CONFIG["useOverlayMetricLabels"],
            )
        ),
        "bankOrder": [
...
def load_config(input_path: Path | None) -> dict[str, Any]:
    ...
    formatted_keys = {
        ...
        "use_overlay_metric_labels",
        "native_segment_labels",
        "use_overlay_segment_labels",
    }
...
def safe_sheet_name(config: dict[str, Any]) -> str:
    raw = str(config.get("sheetName") or config.get("sectionTitle") or "Metric Growth")
    cleaned = re.sub(r"[\[\]:*?/\\]", " ", raw)[:31].strip()
    return cleaned or "Metric Growth"
...
def ordered_legend_segments(config: dict[str, Any]) -> list[dict[str, Any]]:
    raw_order = config.get("legendOrder")
    if not raw_order:
    ...
    for segment in config["segments"]:
        lookup[normalize_text(segment["key"])] = segment
        lookup[normalize_text(segment["label"])] = segment

    ordered = []
    used_keys = set()
    for item in raw_order:
        segment = lookup.get(normalize_text(item))
        if segment and segment["key"] not in used_keys:
            ordered.append(segment)
            used_keys.add(segment["key"])
    ordered.extend(segment for segment in config["segments"] if segment["key"] not in used_keys)
    return ordered


def highlight_bank_names(config: dict[str, Any]) -> set[str]:
    raw_banks = config.get("highlightBanks") or config.get("highlight_banks") or []
    if isinstance(raw_banks, str):
        raw_banks = [raw_banks]
    return {canonical_bank_name(bank) for bank in raw_banks}


def bank_is_highlighted(bank: dict[str, Any], config: dict[str, Any]) -> bool:
    return bool(bank.get("highlighted")) or bank["bank"] in highlight_bank_names(config)


def draw_bank_highlights(
    ...
    highlighted = [
        ...
        if index < len(CENTER_COLS) and bank_is_highlighted(bank, config)
    ]
    if not highlighted:
        return

    box = {**DEFAULT_CONFIG["highlightBox"], **(config.get("highlightBox") or {})}
    width_px = to_number(box.get("widthPx"), DEFAULT_CONFIG["highlightBox"]["widthPx"])
    top_row = int(to_number(box.get("topRow"), DEFAULT_CONFIG["highlightBox"]["topRow"]))
    bottom_row = int(to_number(box.get("bottomRow"), DEFAULT_CONFIG["highlightBox"]["bottomRow"]))
    top_offset_px = to_number(box.get("topOffsetPx"), 0)
    bottom_offset_px = to_number(box.get("bottomOffsetPx"), 0)
    top_px = row_top_px(top_row) + top_offset_px
    height_px = max(1, row_top_px(bottom_row + 1) - top_px + bottom_offset_px)

    for bank_index, _bank in highlighted:
        left_px = category_center_px(bank_index) - width_px / 2
        row, y_offset = pixel_to_row_anchor(top_px)
        col, x_offset = pixel_to_col_anchor(left_px)
        worksheet.insert_textbox(
            ...
            "",
            {
                ...
                "fill": {"none": True},
                "line": {
                    "color": box.get("color", DEFAULT_CONFIG["highlightBox"]["color"]),
                    "width": to_number(
                        box.get("lineWidth"), DEFAULT_CONFIG["highlightBox"]["lineWidth"]
                    ),
                },
                "object_position": 2,
                "decorative": True,
...
def write_legend(
    ...
    segments = ordered_legend_segments(config)
    cursor_col = 1
    for segment in segments:
        legend_format = workbook.add_format({"font_size": 14, "font_color": segment["color"]})
        label = f"{chr(9632)} {segment['label']}"
        span = 1
        if len(segments) <= 3:
            span = max(1, min(4, (len(label) + 9) // 10))
        end_col = cursor_col + span - 1
        if span > 1:
            worksheet.merge_range(1, cursor_col, 1, end_col, label, legend_format)
        else:
            worksheet.write(1, cursor_col, label, legend_format)
...
def make_formats(workbook: xlsxwriter.Workbook) -> dict[str, Any]:
    return {
        ...
        "top_pct": workbook.add_format(
            {"bold": True, "font_size": 13, "align": "center", "num_format": "0%"}
        ),
...
def write_source_table(
    ...
    worksheet.add_table(
        ...
        end_col,
        {
            "name": "AttributedGrowthByMetricSource",
...
def add_native_chart(
    ...
    for index, segment in enumerate(config["segments"]):
        ...
        if config.get("nativeMetricLabels", config.get("nativeSegmentLabels")):
            series["data_labels"] = {
                "custom": [
                    {
                        "value": pct_label(
    ...
    if not config.get("useOverlayMetricLabels", config.get("useOverlaySegmentLabels", True)):
...
def write_attributed_growth_workbook(config: dict[str, Any], output_path: Path) -> None:
    ...
    write_legend(workbook, worksheet, config)
    ...
    with tempfile.TemporaryDirectory() as temp_dir_name:
        ...
        draw_bank_highlights(worksheet, config)
        workbook.close()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build attributed growth by metric workbook.")
